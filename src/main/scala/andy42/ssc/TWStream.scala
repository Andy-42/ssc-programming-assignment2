...
object TWStream {
  ...
  def jsonStream[F[_] : ConcurrentEffect : ContextShift]: Reader[Config, Stream[F, Json]] = Reader { config =>
    ...
    /** Sign the request with OAuth 1.0a API key and access token.
      ...
      */
    def sign(req: Request[F], config: Config): F[Request[F]] = {
    ...
    for {
      ...
      signedRequest <- Stream.eval(sign(request, config))
